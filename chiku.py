...
async def type_and_send(message):
    ...
    response, _ = await gather(chikuQuery(query, user_id), sleep(2))
...
async def repo(_, message):
    await message.reply_text(
        "[GitHub](https://github.com/heyaaman/Chiku_Chatbot)"
...
async def start(_, message):
    await chiku.send_chat_action(message.chat.id, "typing")
    await sleep(2)
